...
//     rawId: prefixedId.slice(idx + 1),      // "69f88324..." | "0"
//   };
// };

// // resolves author display values from any post shape
// const resolveAuthor = (post) => {
//   // fastest path — pre-resolved by BlogCard via Link state
//   if (post._displayName) {
//     return {
//       authorName: post._displayName,
//       userName:   post._displayUserName || "",
//       picture:    pictureMap[post._displayPicture] ?? null,
//     };
//   }

//   // blog post with populated author object (after API fetch)
//   if (post.author && typeof post.author === "object") {
//     const { firstName, lastName, userName, pictures } = post.author;
//     return {
//       authorName: `${firstName ?? ""} ${lastName ?? ""}`.trim() || "Unknown Author",
//       userName:   userName || "",
//       picture:    pictureMap[pictures?.split("/").pop()] ?? null,
//     };
//   }

//   // default post — flat fields
//   return {
//     authorName: `${post.firstName ?? ""} ${post.lastName ?? ""}`.trim() || "Unknown Author",
//     userName:   post.userName || "",
//     picture:    pictureMap[post.pictures?.split("/").pop()] ?? null,
//   };
// };
...
//   // seed state from Link — gives instant render on navigation
//   const [post,    setPost]    = useState(location.state?.post ?? null);
//   const [loading, setLoading] = useState(false);
//   const [error,   setError]   = useState(null);

//   useEffect(() => {
//     // always refetch — covers refresh + direct URL visit
...
//         setPost(data.data ?? data);          // handles { success, data } or raw
//       } catch (err) {
...
//   const { authorName, userName, picture } = resolveAuthor(post);

//   // blog posts: Cloudinary URL (starts with http)
//   // default posts: local asset via imageMap
//   const coverImage = post.image?.startsWith("http")
//     ? post.image
//     : post.coverImage?.startsWith("http")
//       ? post.coverImage
//       : imageMap[post.image?.split("/").pop()] ?? null;

//   const readTime = typeof post.readTime === "number"
//     ? `${post.readTime} min`
//     : (post.readTime ?? "1 min");
...
//                 <span className="font-medium capitalize text-slate-700 dark:text-slate-200">{val}</span>
//               </div>
//             ))}
//           </div>
...
//             <div className="flex items-center gap-3">
//               {picture ? (
//                 <img src={picture} alt={authorName} className="h-10 w-10 rounded-full object-cover" />
//               ) : (
//                 <div className="flex h-10 w-10 items-center justify-center rounded-full bg-violet-200 text-sm font-medium text-violet-800">
//                   {authorName.charAt(0)}
//                 </div>
//               )}
//               <div>
//                 <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{authorName}</p>
//                 <p className="text-xs text-slate-400">{userName}</p>
//               </div>
//             </div>
//           </div>
