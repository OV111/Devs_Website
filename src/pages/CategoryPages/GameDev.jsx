...
const GameDev = () => {
  ...
  useEffect(() => {
    ...
    const load = auth
      ? fetchBlogs(1, 8, { category: "Game Development" }).then(({ blogs }) => blogs)
      : fetchDefaultPostsByCategory("gamedev");
    load
  ...
  return (
    <React.Fragment>
      <header className="min-h-screen pt-40">
        <FloatingIcons category="gamedev" />
        ...
        >
          <span className="bg-linear-to-r from-slate-400 via-purple-600 to-indigo-400 text-center block max-w-3xl mx-auto bg-clip-text text-transparent">
            Bring interactive worlds to life — explore game design patterns,
            physics systems, rendering pipelines, multiplayer architecture,
