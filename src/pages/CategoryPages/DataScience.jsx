...
const DataScience = () => {
  ...
  useEffect(() => {
    ...
    const load = auth
      ? fetchBlogs(1, 8, { category: "Data Science" }).then(({ blogs }) => blogs)
      : fetchDefaultPostsByCategory("datascience");
    load
  ...
  return (
    <React.Fragment>
      <header className="min-h-screen pt-40">
        <FloatingIcons category="datascience" />
        ...
        >
          <span className="bg-linear-to-r from-slate-400 via-purple-600 to-indigo-400 text-center block max-w-3xl mx-auto bg-clip-text text-transparent">
            Turn raw data into actionable insight — master statistical analysis,
            build predictive models, visualize complex datasets, and deploy
            data pipelines that power real decisions.
          </span>
        </motion.p>
      </header>
